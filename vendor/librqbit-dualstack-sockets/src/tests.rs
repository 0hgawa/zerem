...
impl BindSpec {
    fn bind_tcp(&self) -> TcpListener {
        let res = TcpListener::bind_tcp(
            self.addr,
            BindOpts { request_dualstack: self.request_dualstack, ..Default::default() },
        )
        .unwrap();
...
async fn test_tcp(server: BindSpec, tests: &[SendAssertion]) {
    for test in tests.iter().copied() {
        ...
        let remote = match test.spec {
            SendSpec::SendToV4 => SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), server.bind_addr().port()),
            SendSpec::SendToV6 => SocketAddr::new(IpAddr::V6(Ipv6Addr::LOCALHOST), server.bind_addr().port()),
        };
        ...
        let f2 = async {
            ...
            let mut stream = if test.should_work {
                res.with_context(|| format!("error connecting to {remote}")).unwrap()
            } else {
                return;
            };
    ...
    for test in tests.iter().copied() {
        ...
        let f2 = async {
            ...
            let res = timeout(TIMEOUT, server1.send_to(&buf, remote))
                ...
                .unwrap();
            if test.should_work {
                res.with_context(|| format!("error sending to {remote}")).unwrap();
            } else {
                assert!(res.is_err())
            }
        };
...
async fn test_tcp_ipv6_unspecified_dualstack() {
    setup_test_logging();
    test_tcp(
        BindSpec { addr: ipv6_unspecified(), request_dualstack: true, expect_dualstack: true },
        &[
            SendAssertion { spec: SendSpec::SendToV6, should_work: true },
            SendAssertion { spec: SendSpec::SendToV4, should_work: true },
        ],
    )
    .await
}
...
async fn test_tcp_ipv6_unspecified_no_dualstack() {
    setup_test_logging();
    test_tcp(
        BindSpec { addr: ipv6_unspecified(), request_dualstack: false, expect_dualstack: false },
        &[
            SendAssertion { spec: SendSpec::SendToV6, should_work: true },
            SendAssertion { spec: SendSpec::SendToV4, should_work: false },
        ],
    )
    .await
}
...
async fn test_tcp_ipv6_localhost() {
    setup_test_logging();
    test_tcp(
        BindSpec { addr: ipv6_localhost(), request_dualstack: true, expect_dualstack: false },
        &[
            SendAssertion { spec: SendSpec::SendToV6, should_work: true },
            SendAssertion { spec: SendSpec::SendToV4, should_work: false },
        ],
...
async fn test_tcp_ipv4_localhost() {
    setup_test_logging();
    test_tcp(
        BindSpec { addr: ipv4_localhost(), request_dualstack: true, expect_dualstack: false },
        &[
            SendAssertion { spec: SendSpec::SendToV6, should_work: false },
            SendAssertion { spec: SendSpec::SendToV4, should_work: true },
        ],
    )
    .await
...
async fn test_udp_ipv6_localhost() {
    ...
    test_udp(
        BindSpec { addr: ipv6_localhost(), request_dualstack: true, expect_dualstack: false },
        BindSpec { addr: ipv6_localhost(), request_dualstack: false, expect_dualstack: false },
        &[
            SendAssertion { spec: SendSpec::SendToV6, should_work: true },
            SendAssertion { spec: SendSpec::SendToV4, should_work: false },
        ],
...
async fn test_tcp_from_fd_wrong_socket() {
    ...
    assert!(TcpListener::try_from(fd).is_err(), "should not convert a UDP socket into a TCP listener",);
}
