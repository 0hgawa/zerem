...
async fn bind_mcast_sock(port: u16, bd_name: Option<&str>) -> MulticastUdpSocket {
    ...
    MulticastUdpSocket::new(
        ...
        Some(SocketAddrV6::new(Ipv6Addr::new(0xff02, 0, 0, 0, 0, 0, 0, 0xc), port, 0, 0)),
        bd.as_ref(),
    )
    .await
    .unwrap()
...
async fn test_v4_received() {
    ...
    sock.try_send_mcast_everywhere(&|opts| {
        if opts.iface_ip().is_ipv4() { Some("hello".into()) } else { None }
    })
    .await;

    let mut buf = [0u8; 5];
    let (sz, addr) = timeout(Duration::from_millis(100), sock.recv_from(&mut buf)).await.unwrap().unwrap();
    assert_eq!(sz, 5);
...
async fn test_v6_received() {
    ...
    sock.try_send_mcast_everywhere(&|opts| {
        if opts.iface_ip().is_ipv6() { Some("hello".into()) } else { None }
    })
    .await;
...
async fn bind_multiple_same_port() {
    ...
    let mut buf = [0u8; 5];
    let (sz, addr) = timeout(Duration::from_millis(100), sock1.recv_from(&mut buf)).await.unwrap().unwrap();
    assert_eq!(sz, 5);
    assert!(addr.is_ipv4(), "{addr:?} expected v4");
    assert_eq!(&buf, b"hello");

    let (sz, addr) = timeout(Duration::from_millis(100), sock2.recv_from(&mut buf)).await.unwrap().unwrap();
    assert_eq!(sz, 5);
    assert!(addr.is_ipv4(), "{addr:?} expected v4");
    assert_eq!(&buf, b"hello");
}
...
async fn test_mcast_bind_device() {
    ...
    sock.try_send_mcast_everywhere(&|_| Some("hello".into())).await;
