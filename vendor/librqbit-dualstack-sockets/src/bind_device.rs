...
impl BindDevice {
    ...
    #[cfg(windows)]
    // NOT UPSTREAM: the argument is underscored. It is unused in this stub, and
    // a vendored crate is a path dependency, which gets no --cap-lints to hide
    // the warning behind.
    ...
    pub fn bind_sref(&self, sref: &socket2::Socket, is_v6: bool) -> crate::Result<()> {
        if is_v6 {
            sref.bind_device_by_index_v6(Some(self.index)).map_err(Error::BindDeviceSetDeviceError)
        } else {
            sref.bind_device_by_index_v4(Some(self.index)).map_err(Error::BindDeviceSetDeviceError)
        }
    }
    ...
    pub fn bind_sref(&self, sref: &socket2::Socket, _is_v6: bool) -> crate::Result<()> {
        ...
        sref.bind_device(Some(name)).map_err(Error::BindDeviceSetDeviceError)
    }
