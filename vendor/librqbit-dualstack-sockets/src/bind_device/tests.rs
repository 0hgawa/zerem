...
async fn test_bind_to_device() {
    ...
    let res = timeout(
        TIMEOUT,
        tcp_connect(test_addr, ConnectOpts { bind_device: Some(&bd), ..Default::default() }),
    )
    .await;
    ...
    match &res {
        ...
        Err(_) => {
            println!("timeout, this is expected on linux, as SO_BINDTODEVICE would route the packet via lo")
        }
    }
}
