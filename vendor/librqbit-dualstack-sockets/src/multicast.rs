...
impl MulticastUdpSocket {
    pub async fn new(
        ...
        let opts = BindOpts { request_dualstack: true, reuseport: true, device: bind_device };
    ...
    async fn bind_multicast(&self) -> crate::Result<()> {
        ...
        for nic in self.nics.iter() {
            ...
            for addr in nic.addr.iter() {
                match (addr.ip(), self.sock.bind_addr().is_ipv6()) {
                    ...
                    {
                        let is_linux_or_windows = cfg!(any(target_os = "linux", target_os = "windows"));
                        if !is_ipv6 || is_linux_or_windows {
                            joined |= try_join_v4(&self.sock, *self.ipv4_addr.ip(), iface_addr);
                        } else {
                            joined |= try_join_v6(&self.sock, self.ipv4_addr.ip().to_ipv6_mapped(), nic.index)
                        }
                    }
                    (IpAddr::V6(addr), true) => {
        ...
        self.sock.socket().writable().await.map_err(Error::Writeable)?;

        Ok(())
    }

    pub fn find_mcast_opts_for_replying_to(&self, addr: &SocketAddr) -> Option<MulticastOpts> {
        self.nics().iter().flat_map(|nic| nic.addr.iter().map(move |addr| (nic, addr))).find_map(
            |(nic, naddr)| {
                ...
                let mcast_addr: SocketAddr = match (addr, naddr.ip(), nm, self.ipv6_link_local) {
                    ...
                    // address
                    (SocketAddr::V6(addr), _, _, Some(mlocal)) if addr.ip().is_unicast_link_local() => {
                    ...
                    (SocketAddr::V6(addr), IpAddr::V6(naddr), Some(IpAddr::V6(mask)), _)
                        if addr.ip().is_unique_local()
                            && addr.ip().to_bits() & mask.to_bits() == naddr.to_bits() & mask.to_bits() =>
                    {
                    ...
                    (SocketAddr::V4(addr), IpAddr::V4(naddr), Some(IpAddr::V4(mask)), _)
                        if addr.ip().to_bits() & mask.to_bits() == naddr.to_bits() & mask.to_bits() =>
                    {
                        self.ipv4_addr.into()
                    }
                    _ => return None,
                };
                Some(MulticastOpts { interface_id: nic.index, interface_addr: naddr.ip(), mcast_addr })
            },
        )
    }

    pub async fn send_multicast_msg(&self, buf: &[u8], opts: &MulticastOpts) -> crate::Result<usize> {
        ...
        poll_fn(|cx| {
            ...
            match (opts.mcast_addr(), opts.iface_ip(), bind_is_ipv6, is_linux) {
                ...
                | (SocketAddr::V4(_), IpAddr::V4(addr), false, false) => {
                    sref.set_multicast_if_v4(&addr).map_err(Error::SetMulticastIpv4)?;
                }
                (SocketAddr::V6(_), IpAddr::V6(_), _, _) | (SocketAddr::V4(_), IpAddr::V4(_), _, _) => {
                    sref.set_multicast_if_v6(opts.interface_id).map_err(Error::SetMulticastIpv6)?;
                }
                _ => return Poll::Ready(Err(Error::SendMulticastMsgProtocolMismatch)),
            }

            self.sock.poll_send_to(cx, buf, opts.mcast_addr).map_err(Error::Send)
        })
        .await
    }
        ...
        let mut send_specs = self
            ...
            .filter_map(|(ifidx, ifaddr)| {
                let mcast_addr: SocketAddr = match (bind_is_ipv6, ifaddr, self.ipv6_link_local) {
                    (_, IpAddr::V4(a), _) if a.is_private() || a.is_loopback() => self.ipv4_addr.into(),
                ...
                Some(MulticastOpts { interface_id: ifidx, interface_addr: ifaddr, mcast_addr })
            })
            .collect::<Vec<_>>();
